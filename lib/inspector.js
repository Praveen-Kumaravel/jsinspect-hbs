...
class Inspector extends EventEmitter {
  ...
  constructor(filePaths, opts) {
    ...
    NodeUtils.mode = opts.mode || 'strict';
  ...
  _expand(nodeArrays) {
    var traversals = nodeArrays.map(nodes => {
      return this._traversals[nodes[0].loc.module];
    });

    var headPositions = nodeArrays.map((nodes, i) => {
      return traversals[i] && traversals[i].indexOf(nodes[0]);
    });

    var tailPositions = nodeArrays.map((nodes, i) => {
      var last = nodes[nodes.length - 1];
      return traversals[i] && traversals[i].indexOf(last);
    });
