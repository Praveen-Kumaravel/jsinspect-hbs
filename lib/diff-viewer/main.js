...
function updateDiffContent(contents, isB = false) {
...
function setupInstancePickers(similarityId) {
    ...
    updateDiffContent(similarity.instances[0].code);
    updateDiffContent(similarity.instances[1].code, true);
    changed();
}
...
function getCodeBlockSelectHandler(isB = false) {
    return function (event) {
        const {code} = selectedSimilarity.instances.find(({path}) => path === event.target.value);
        updateDiffContent(code, isB);
        changed();
    }
}
