...
function updateDiffContent(contents, lines, isB = false) {
...
function setupInstancePickers(similarityId) {
    ...
    updateDiffContent(similarity.instances[0].code, similarity.instances[0].lines);
    updateDiffContent(similarity.instances[1].code, similarity.instances[1].lines, true);
    changed();
}
...
function getCodeBlockSelectHandler(isB = false) {
    return function (event) {
        const {code, lines} = selectedSimilarity.instances.find(({path}) => path === event.target.value);
        updateDiffContent(code, lines, isB);
        changed();
    }
}
