...
describe('ember-template-recast', function () {
    describe('ElementNode', function () {
        ...
        test('updating attributes on a non-self-closing void element', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`<img src="{{this.something}}">`);
        });
        test('reusing another template part to build a new template', function () {
            let template = `foo`;
            let original = (0, _1.parse)(template, sampleFilename);
            let text = original.body[0];
            let ast = _1.builders.template([text]);
            expect((0, _1.print)(ast)).toEqual(`foo`);
        });
        test('wrapping a parsed node (which uses custom formatting) with a raw node', function () {
            ...
            let original = (0, _1.parse)(template, sampleFilename);
            let raw_wrapping_ast = _1.builders.template([
                _1.builders.element('div', {
                    children: original.body,
                }),
            ]);
            expect((0, _1.print)(raw_wrapping_ast)).toEqual(`<div><Foo @class='a {{b}} c' /></div>`);
        });
        test('changing an element to a void element does not print closing tag', function () {
            ...
            let element = ast.body[0];
            element.tag = 'img';
            expect((0, _1.print)(ast)).toEqual(`<img data-foo="{{something}}">`);
        });
        ...
        test('changing an attribute value from mustache to text node (GH#111)', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`<FooBar @thing="static thing 1" @baz="static thing 2" />`);
        });
        test('changing an attribute value from text node to mustache (GH #139)', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`<FooBar @foo={{my-awesome-helper "hello" "world"}} />`);
        });
        test('changing an attribute value from text node to concat statement (GH #139)', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`<FooBar @foo="Hello {{my-awesome-helper "hello" "world"}} world" />`);
        });
      ...
      <div data-foo='single quoted'>
            ...
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].tag = 'a';
            expect((0, _1.print)(ast)).toEqual((0, outdent_1.default) `
      ...
      <div></div>`;
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].tag = 'a';
            expect((0, _1.print)(ast)).toEqual(`<a></a>`);
        });
        test('rename self-closing element tagname', function () {
            let ast = (0, _1.parse)('<Foo bar="baz"/>', sampleFilename);
            ast.body[0].tag = 'Qux';
            expect((0, _1.print)(ast)).toEqual('<Qux bar="baz"/>');
        });
        test('rename self-closing element tagname with trailing whitespace', function () {
            let ast = (0, _1.parse)('<Foo />', sampleFilename);
            ast.body[0].tag = 'Qux';
            expect((0, _1.print)(ast)).toEqual('<Qux />');
        });
        test('Rename tag and convert from self-closing with attributes to block element', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<Qux bar="baz">bay</Qux>');
        });
        test('convert from self-closing with attributes to block element', function () {
            let ast = (0, _1.parse)('<Foo bar="baz" />', sampleFilename);
            ast.body[0].children = [_1.builders.text('bay')];
            expect((0, _1.print)(ast)).toEqual('<Foo bar="baz">bay</Foo>');
        });
        test('convert from self-closing with specially spaced attributes to block element', function () {
            let ast = (0, _1.parse)('<Foo\n  bar="baz"\n />', sampleFilename);
            ast.body[0].children = [_1.builders.text('bay')];
            expect((0, _1.print)(ast)).toEqual('<Foo\n  bar="baz"\n >bay</Foo>');
        });
        test('Convert self-closing element with modifiers block element', function () {
            let ast = (0, _1.parse)('<Foo {{on "click" this.doSomething}} />', sampleFilename);
            ast.body[0].children = [_1.builders.text('bay')];
            expect((0, _1.print)(ast)).toEqual('<Foo {{on "click" this.doSomething}}>bay</Foo>');
        });
      ...
      ></div>`);
        ...
        test('creating an element with complex attributes', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`<FooBar @thing={{hash something=bar}} />`);
        });
        ...
        test('modifying attribute after valueless attribute', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<Foo data-foo data-derp={{this.hmmm}} />');
        });
        ...
        test('adding attribute after valueless attribute', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<Foo data-foo data-bar="foo" />');
        });
        test('adding valueless attribute when no open parts existed', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<Foo data-bar />');
        });
      ...
      <div class="foo" {{on "click" this.foo}}></div>`);
        ...
        test('adding modifier when ...attributes is present', function () {
            let template = (0, outdent_1.default) `<div data-foo="asdf" data-foo data-other="asdf"></div>`;
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].modifiers.push(_1.builders.elementModifier('on', [_1.builders.string('click'), _1.builders.path('this.foo')]));
            expect((0, _1.print)(ast)).toEqual(`<div data-foo="asdf" data-foo data-other="asdf" {{on "click" this.foo}}></div>`);
        });
      ...
      <div class="foo" {{!-- template-lint-disable --}}></div>`);
        ...
        test('adding block param', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`<MyFoo class="foo" as |blah|></MyFoo>`);
        });
        test('removing a block param', function () {
            let template = `<MyFoo class="foo" as |bar|></MyFoo>`;
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].blockParams.pop();
            expect((0, _1.print)(ast)).toEqual(`<MyFoo class="foo"></MyFoo>`);
        });
      ...
      ></MyFoo>`);
        ...
        test('interleaved attributes and modifiers are not modified when unchanged', function () {
            ...
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].comments.push(_1.builders.mustacheComment(' template-lint-disable '));
            expect((0, _1.print)(ast)).toEqual(`<div data-test="foo" {{on "click" this.bar}} data-blah="derp" {{!-- template-lint-disable --}}></div>`);
        });
      ...
      `);
        ...
        test('adding children to an empty element', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<div>some text</div>');
        });
        test('adding children to a self closing element', function () {
            let template = `<Foo />`;
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].children.push(_1.builders.text('some text'));
            expect((0, _1.print)(ast)).toEqual('<Foo>some text</Foo>');
        });
...
>
  ...
  {{/each}}
            ...
            let ast = (0, _1.parse)(template, sampleFilename);
            let block1 = ast.body[0].children[1];
            let block2 = block1.program.body[1];
            let block3 = block2.program.body[1];
            let element = block3.program.body[1];
            let attribute = element.attributes[0];
            attribute.value.chars = 'foo';
            expect((0, _1.print)(ast)).toEqual(`<div
  class="pt-2 pb-4 {{this.foo}}"
>
  {{#each this.data as |chunks|}}
    {{#each chunks as |chunk|}}
  ...
  {{/each}}
    ...
    describe('MustacheStatement', function () {
        test('path mutations retain custom whitespace formatting', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('{{ bar }}');
        });
      ...
      {{baz-derp
        ...
        {{ other-stuff }}
      `;
            let { code } = (0, _1.transform)(template, (env) => {
        ...
        test('replacing empty hash pair on MustacheStatement works', function () {
            ...
            expect((0, _1.print)(ast)).toEqual((0, outdent_1.default) `{{foo-bar hello="world"}}`);
        });
    ...
    describe('SubExpression', function () {
        ...
        {{foo-bar
          baz=(stuff)
            ...
            expect((0, _1.print)(ast)).toEqual(`{{foo-bar\n  baz=(stuff hello="world")\n}}`);
        });
    });
    describe('BlockStatement', function () {
      ...
      {{/baz-derp}}{{someInlineComponent hello="world"}}`);
        ...
        test('replacing a previously empty hash', function () {
            ...
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].hash = _1.builders.hash([_1.builders.pair('hello', _1.builders.string('world'))]);
            expect((0, _1.print)(ast)).toEqual('{{#foo-bar hello="world"}}Hi there!{{/foo-bar}}');
        });
        test('adding multiple HashPair to previously empty hash', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('{{#foo-bar hello="world" foo="bar"}}Hi there!{{/foo-bar}}{{baz}}');
        });
        test('replacing empty hash w/ block params works', function () {
            let template = `{{#foo-bar as |a b c|}}Hi there!{{/foo-bar}}`;
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].hash = _1.builders.hash([_1.builders.pair('hello', _1.builders.string('world'))]);
            expect((0, _1.print)(ast)).toEqual('{{#foo-bar hello="world" as |a b c|}}Hi there!{{/foo-bar}}');
        });
        ...
        test('changing a HashPair key with a StringLiteral value (GH#112)', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('{{#foo-bar bar="some thing with a space"}}Hi there!{{/foo-bar}}');
        });
        test('changing a HashPair key with a SubExpression value (GH#112)', function () {
            ...
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].hash.pairs[0].key = 'bar';
            expect((0, _1.print)(ast)).toEqual('{{#foo-bar bar=(helper-here this.arg1 this.arg2)}}Hi there!{{/foo-bar}}');
        });
        test('changing a HashPair value from StringLiteral to SubExpression', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('{{#foo-bar foo=(concat "hello" "world")}}Hi there!{{/foo-bar}}');
        });
        ...
        test('adding child to end of program', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('{{#foo-bar}}Hello world!{{/foo-bar}}');
        });
        test('adding child to beginning of program', function () {
            let template = `{{#foo-bar}}Hello{{/foo-bar}}`;
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].program.body.unshift(_1.builders.text('ZOMG! '));
            expect((0, _1.print)(ast)).toEqual('{{#foo-bar}}ZOMG! Hello{{/foo-bar}}');
        });
        test('adding child to end of inverse', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('{{#foo-bar}}{{else}}Hello world!{{/foo-bar}}');
        });
        test('adding child to beginning of inverse', function () {
            let template = `{{#foo-bar}}{{else}}Hello{{/foo-bar}}`;
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].inverse.body.unshift(_1.builders.text('ZOMG! '));
            expect((0, _1.print)(ast)).toEqual('{{#foo-bar}}{{else}}ZOMG! Hello{{/foo-bar}}');
        });
        test('adding child to end of inverse preserves whitespace and whitespace control when program is also present', function () {
            ...
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].inverse.body.push(_1.builders.text(' world!'));
            expect((0, _1.print)(ast)).toEqual('{{#foo-bar}}Goodbye\n  {{~ else ~}} Hello world!{{/foo-bar}}');
        });
    ...
    describe('AttrNode', function () {
        test('updating value', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<Foo bar={{bar}} />');
        });
        test('updating attribute to be valueless', function () {
            let template = '<Foo data-foo="bar" />';
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].attributes[0].isValueless = true;
            expect((0, _1.print)(ast)).toEqual('<Foo data-foo />');
            ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].attributes[0].isValueless = true;
            ast.body[0].attributes[0].value = _1.builders.text('blah');
            expect((0, _1.print)(ast)).toEqual('<Foo data-foo />');
        });
        test('adding value to valueless attribute', function () {
            let template = '<Foo data-foo />';
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].attributes[0].value = _1.builders.mustache('true');
            expect((0, _1.print)(ast)).toEqual('<Foo data-foo={{true}} />');
        });
        ...
        test('updating value from non-quotable to ConcatStatement (GH#111)', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<Foo bar="{{foo}} static {{bar}}" />');
        });
        test('can determine if an AttrNode was valueless (required by ember-template-lint)', function () {
            expect((0, _1.parse)(`<Foo bar={{foo}} />`, sampleFilename).body[0].attributes[0].isValueless).toBe(false);
            expect((0, _1.parse)(`<Foo bar="foo {{bar}}" />`, sampleFilename).body[0].attributes[0].isValueless).toBe(false);
            expect((0, _1.parse)(`<Foo bar='foo {{bar}}' />`, sampleFilename).body[0].attributes[0].isValueless).toBe(false);
            expect((0, _1.parse)(`<Foo bar="foo" />`, sampleFilename).body[0].attributes[0].isValueless).toBe(false);
            expect((0, _1.parse)(`<Foo bar='foo' />`, sampleFilename).body[0].attributes[0].isValueless).toBe(false);
            expect((0, _1.parse)(`<Foo bar=foo />`, sampleFilename).body[0].attributes[0].isValueless).toBe(false);
            expect((0, _1.parse)(`<Foo bar />`, sampleFilename).body[0].attributes[0].isValueless).toBe(true);
        });
        test('can determine type of quotes used from AST (required by ember-template-lint)', function () {
            expect((0, _1.parse)(`<Foo bar={{foo}} />`, sampleFilename).body[0].attributes[0].quoteType).toBe(null);
            expect((0, _1.parse)(`<Foo bar="foo {{bar}}" />`, sampleFilename).body[0].attributes[0].quoteType).toBe(`"`);
            expect((0, _1.parse)(`<Foo bar='foo {{bar}}' />`, sampleFilename).body[0].attributes[0].quoteType).toBe(`'`);
            expect((0, _1.parse)(`<Foo bar="foo" />`, sampleFilename).body[0].attributes[0].quoteType).toBe(`"`);
            expect((0, _1.parse)(`<Foo bar='foo' />`, sampleFilename).body[0].attributes[0].quoteType).toBe(`'`);
            expect((0, _1.parse)(`<Foo bar=foo />`, sampleFilename).body[0].attributes[0].quoteType).toBe(null);
            expect((0, _1.parse)(`<Foo bar />`, sampleFilename).body[0].attributes[0].quoteType).toBe(null);
        });
      ...
      `;
            ...
            expect((0, _1.print)(ast)).toEqual(template);
        });
        test('quotes are preserved when updated a TextNode value (double quote)', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<div class="hahah"></div>');
        });
        ...
        test('can update a quoteless attribute value', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<div class=zomgyasss></div>');
        });
        test('quoteless attribute values can be updated to a must-quote attribute value', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<div class="foo bar baz"></div>');
        });
        test('quotes are preserved when updating a ConcatStatement value', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<div class="hahah {{foo}}"></div>');
        });
        test('quotes are preserved when updating an AttrNode name - issue #319', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<div class="{{if foo "bar"}} baz" />');
        });
        test('quotes are preserved when updating an AttrNode value - issue #588', function () {
            let template = '<div class="{{if foo "bar"}} baz" />';
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].attributes[0].value = _1.builders.concat([_1.builders.text('foobar')]);
            expect((0, _1.print)(ast)).toEqual('<div class="foobar" />');
        });
        test('TextNode quote types can be changed', function () {
            let template = '<div class=foo />';
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].attributes[0].quoteType = "'";
            expect((0, _1.print)(ast)).toEqual("<div class='foo' />");
            ast.body[0].attributes[0].quoteType = '"';
            expect((0, _1.print)(ast)).toEqual('<div class="foo" />');
        ...
        test('ConcatStatement quote types can be changed', function () {
            ...
            let ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].attributes[0].quoteType = "'";
            expect((0, _1.print)(ast)).toEqual("<div class='{{foo}} static {{bar}}' />");
        ...
        test('can specify quote style on a new attribute', function () {
            let template = '<div />';
            let ast = (0, _1.parse)(template, sampleFilename);
            let c = _1.builders.attr('class', _1.builders.text('foo'));
            c.quoteType = null;
        ...
        test('can specify valueless on a new attribute', function () {
            let template = '<div />';
            let ast = (0, _1.parse)(template, sampleFilename);
            let c = _1.builders.attr('...attributes', _1.builders.text(''));
            c.isValueless = true;
            ast.body[0].attributes.push(c);
            expect((0, _1.print)(ast)).toEqual('<div ...attributes />');
        });
        test('invalid quote types are rejected', function () {
            ...
            ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].attributes[0].quoteType = '"';
            ast.body[0].attributes[0].value = _1.builders.mustache('foo');
            expect(() => (0, _1.print)(ast)).toThrow('should not be quoted');
            ast.body[0].attributes[0].quoteType = "'";
            expect(() => (0, _1.print)(ast)).toThrow('should not be quoted');
            ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].attributes[0].quoteType = null;
            ...
            expect(() => (0, _1.print)(ast)).toThrow('must be quoted');
            ast = (0, _1.parse)(template, sampleFilename);
            ast.body[0].attributes[0].quoteType = null;
            ast.body[0].attributes[0].value = _1.builders.text('foo bar');
            expect(() => (0, _1.print)(ast)).toThrow('`foo bar` is invalid as an unquoted attribute');
        });
    });
    describe('HashPair', function () {
        test('mutations', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('{{foo-bar bar=bar}}');
        });
    ...
    describe('NullLiteral', function () {
        test('it should print correctly', function () {
            ...
            const param = mustache.params[0];
            // Mark the param as dirty
            const oldType = param.type;
            param.type = 'ElementNode';
            param.type = oldType;
            expect((0, _1.print)(ast)).toEqual(`{{contact-null\n  null\n}}`);
        });
    });
    ...
    {{ other-stuff }}
    `;
        let { code } = (0, _1.transform)(template, () => {
            return {
                ElementNode() {
                    return null;
                },
    ...
    describe('MustacheCommentStatement', function () {
        test('can be updated', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`<div {{!-- otherthing --}}></div>`);
        });
    ...
    describe('ElementModifierStatement', function () {
        test('can be updated', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`<div {{other 'foo'}}></div>`);
        });
    });
    describe('CommentStatement', function () {
        test('can be updated', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`<!-- otherthing -->`);
        });
    });
    ...
    describe('StringLiteral', function () {
        test('can be updated', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`{{foo "derp"}}`);
        });
        test('can determine type of quotes used from AST (required by ember-template-lint)', function () {
            expect((0, _1.parse)(`{{foo "blah"}}`, sampleFilename).body[0].params[0].quoteType).toBe('"');
            expect((0, _1.parse)(`{{foo 'blah'}}`, sampleFilename).body[0].params[0].quoteType).toBe("'");
        });
        test('can update quote style', function () {
            let ast = (0, _1.parse)(`{{foo "blah"}}`, sampleFilename);
            ast.body[0].params[0].quoteType = "'";
            expect((0, _1.print)(ast)).toEqual(`{{foo 'blah'}}`);
            ast = (0, _1.parse)(`{{foo 'blah'}}`, sampleFilename);
            ast.body[0].params[0].quoteType = '"';
            expect((0, _1.print)(ast)).toEqual(`{{foo "blah"}}`);
        });
        test('can specify quote style on a new string literal', function () {
            let ast = (0, _1.parse)(`{{foo}}`, sampleFilename);
            let s = _1.builders.string('blah');
            s.quoteType = "'";
    ...
    describe('NumberLiteral', function () {
        test('can be updated', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`{{foo 0}}`);
        });
    });
    describe('BooleanLiteral', function () {
        test('can be updated in MustacheStatement .path position', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`{{false}}`);
        });
        test('can be updated in MustacheStatement .hash position', function () {
            ...
            expect((0, _1.print)(ast)).toEqual(`{{foo thing=false}}`);
        });
    });
    describe('TextNode', function () {
        ...
        test('an AttrNode values quotes are removed when inserted in alternate positions (e.g. content)', function () {
            ...
            expect((0, _1.print)(ast)).toEqual('<div class="lol">lol</div>');
        });
    });
});
